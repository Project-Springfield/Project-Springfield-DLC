...
uniform lowp vec4 vec4Param1; // tint
uniform lowp vec4 vec4Param2; // x is constrast, y is brightness
...
void main()
{
    ...
    // Apply contrast
    lDiffuseColour.rgb = ((lDiffuseColour.rgb - (0.5 * lDiffuseColour.a)) * vec4Param2.x) + (0.5 *lDiffuseColour.a);
    
    // Apply Brightness
    lDiffuseColour.rgb = lDiffuseColour.rgb + (vec4Param2.y * lDiffuseColour.a);
